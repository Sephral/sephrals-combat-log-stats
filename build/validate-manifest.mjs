...
function ensureString(value, fieldName) {
  if (typeof value !== "string" || value.trim().length === 0) fail(`module.json field "${fieldName}" must be a non-empty string.`);
}

function ensureArray(value, fieldName) {
  if (!Array.isArray(value) || value.length === 0) fail(`module.json field "${fieldName}" must be a non-empty array.`);
}
...
if (packageJson.version !== moduleManifest.version) fail(`package.json version (${packageJson.version}) does not match module.json version (${moduleManifest.version}).`);

if (typeof moduleManifest.compatibility !== "object" || moduleManifest.compatibility === null) fail("module.json field \"compatibility\" must be an object.");
